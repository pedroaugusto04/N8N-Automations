...
let IngestEntryUseCase = class IngestEntryUseCase {
    contentRepository;
    constructor(contentRepository) {
        this.contentRepository = contentRepository;
    }
    async execute(input, userId, workspaceSlug = '') {
        return saveIngestedNote(this.contentRepository, userId, input, workspaceSlug);
    }
};
IngestEntryUseCase = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [ContentRepository])
], IngestEntryUseCase);
export { IngestEntryUseCase };
function projectFromPayload(payload, workspaceSlug) {
...
async function saveIngestedNote(contentRepository, userId, input, workspaceSlugOverride = '') {
    ...
    const attachments = await Promise.all(payload.content.attachments.map((attachment) => contentRepository.saveAttachment(userId, {
        noteId: note.id,
