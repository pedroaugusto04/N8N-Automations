...
async function runChatCompletion(config, systemPrompt, userContent) {
...
export async function generateReviewAnalysis(config, promptPayload) {
    const fallback = {
    ...
    if (config.provider === AiProvider.None || !config.apiKey || !config.model)
        return fallback;
    const content = await runChatCompletion(config, [
    ...
    ].join(' '), JSON.stringify(promptPayload));
    if (!content)
        return fallback;
    ...
    return {
        ...
        reviewFindings: Array.isArray(parsed.reviewFindings)
            ? parsed.reviewFindings
                ...
                .map((item) => ({
                severity: Object.values(ReviewFindingSeverity).includes(item.severity)
                    ? item.severity
...
export async function extractConversationFields(config, payload) {
    const content = await runChatCompletion(config, [
    ...
    ].join(' '), payload.messageText);
    if (!content)
        return null;
    const parsed = JSON.parse(content);
    return parsed && typeof parsed === 'object' ? parsed : null;
}
export async function answerKnowledgeQuery(config, payload) {
    const content = await runChatCompletion(config, [
